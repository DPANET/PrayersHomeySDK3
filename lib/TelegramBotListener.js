 ...
 *   pk|N             — pick item N from the active search menu (no Claude)
 *   sp|type|off      — next page of search type at offset off (no Claude)
 *   mr|type[|query]  — "more" — deterministic: with query → search menu; without → random
 *   ex|h[|id]        — explain a hadith — deterministic selection, LLM generation
 *   sz|fatwa|id      — summarize a fatwa by id — LLM generation
 *   sz|he|id         — summarize a hadith explanation by id — LLM generation
...
// Build the callback_data for a "More" button. Always encodes the content type;
// encodes the query when present, truncated to fit Telegram's 64-byte limit.
function moreCallbackData(type, query) {
  const base = 'mr|' + type;
  if (!query) return base;
  const prefix = base + '|';
  const budget = 64 - Buffer.byteLength(prefix, 'utf8');
  if (budget <= 0) return base;
  let q = String(query).replace(/\|/g, '');   // strip pipe separators
  while (q.length > 0 && Buffer.byteLength(q, 'utf8') > budget) q = q.slice(0, q.length - 1);
  return q ? prefix + q : base;
}
function buildReplyMarkup(meta) {
  ...
  if (type === 'verse') {
    ...
    rows.push([
      { text: '⬅️ Prev',  callback_data: `vp|${ref}` },
      { text: '➡️ Next',  callback_data: `vn|${ref}` },
    ]);
    rows.push([
      { text: '🕋 Asbab',  callback_data: `ab|${ref}` },
      { text: '➡️ More',   callback_data: moreCallbackData('quran', meta.query) },
    ]);
  } else if (type === 'tafsir') {
    const ref = `${meta.surahNum}:${meta.ayahNum}`;
    rows.push([
    ...
    // Encode the hadith id so Explain targets THIS hadith even when the user
    // scrolls back to an older card (ex|h is the legacy/no-id fallback).
    const exData = meta.id ? `ex|h|${meta.id}` : 'ex|h';
    rows.push([
      { text: '💡 Explain',  callback_data: exData },
      { text: '➡️ More',     callback_data: moreCallbackData('hadith', meta.query) },
    ]);
  } else if (type === 'hadith_explained') {
    const szHeData = meta.id ? `sz|he|${meta.id}` : 'sz|he';
    rows.push([
      { text: '📝 Summarize', callback_data: szHeData },
      { text: '➡️ More',      callback_data: moreCallbackData('hadith_explained', meta.query) },
    ]);
  } else if (type === 'dua') {
    const row = [];
    ...
    row.push({ text: '➡️ More', callback_data: moreCallbackData('dua', meta.query) });
    rows.push(row);
  } else if (type === 'fatwa') {
    const szFatwaData = meta.id ? `sz|fatwa|${meta.id}` : 'sz|fatwa';
    rows.push([
      { text: '📝 Summarize', callback_data: szFatwaData },
      { text: '➡️ More',      callback_data: moreCallbackData('fatwa', meta.query) },
    ]);
  } else if (type === 'menu') {
    rows.push([
    ...
    // One tap per result (pk|n) — selection is resolved in code without Claude.
    const results = meta.results || [];
    if (results.length) {
      rows.push(results.map(r => ({ text: String(r.n), callback_data: `pk|${r.n}` })));
    }
    // Next-page button (sp) — paging is also code-only, no Claude round-trip.
    if (meta.hasMore && meta.searchType && meta.nextOffset != null) {
      rows.push([{ text: '⬇️ Next 5', callback_data: `sp|${meta.searchType}|${meta.nextOffset}` }]);
    }
  }
...
module.exports.moreCallbackData    = moreCallbackData;
