...
function buildReplyMarkup(meta) {
  ...
  if (type === 'verse') {
    ...
    const row2 = [];
    if (meta.surahNum) row2.push({ text: '📿 Full surah', url: `https://quran.com/${meta.surahNum}` });
    row2.push({ text: '🔁 Another', callback_data: 'nx|q' });
    rows.push(row2);
  } else if (type === 'tafsir') {
    const ref = `${meta.surahNum}:${meta.ayahNum}`;
    rows.push([
    ...
    rows.push([
      ...
      { text: '🔁 Another',  callback_data: 'nx|h' },
    ]);
    if (meta.url) rows.push([{ text: '🔗 Source', url: meta.url }]);
  } else if (type === 'hadith_explained') {
    rows.push([{ text: '🔁 Another', callback_data: 'nx|he' }]);
  } else if (type === 'dua') {
    const row = [];
    ...
    row.push({ text: '🔁 Another', callback_data: 'nx|d' });
    rows.push(row);
  } else if (type === 'fatwa') {
    const row = [];
    if (meta.url) row.push({ text: '🔗 Read full', url: meta.url });
    if (meta.categories && meta.categories.length) {
      const cat = String(meta.categories[0]).slice(0, 40);
      row.push({ text: '➡️ More', callback_data: `nf|${cat}` });
    }
