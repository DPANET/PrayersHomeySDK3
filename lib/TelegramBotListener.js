 ...
 *   ex|h             — explain the last hadith — deterministic selection, LLM generation
...
function buildReplyMarkup(meta) {
  ...
  if (type === 'verse') {
    ...
    rows.push([{ text: '➡️ More', callback_data: moreCallbackData('hadith_explained', meta.query) }]);
  } else if (type === 'dua') {
    const row = [];
    ...
    rows.push([{ text: '➡️ More', callback_data: moreCallbackData('fatwa', meta.query) }]);
  } else if (type === 'menu') {
    rows.push([
