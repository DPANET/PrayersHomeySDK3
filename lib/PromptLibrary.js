...
const DEFAULT_PRESETS = [
  // ── Daily content (the everyday drivers) ──────────────────────────────────
  {
    ...
    prompt: 'A random selector for THIS run is: {{RANDOM4}} (0 = hadith, 1 = Quran, 2 = dua, 3 = fatwa). '
      ...
      + '2 → a random dua/adhkar via get_dua with query "random";\n'
  ...
  },
  {
    id: 'dua_of_the_day',
    name: 'Dua of the day',
    // Themes are situational adhkar that each resolve to a real Hisn al-Muslim
    // chapter (the book is organised by occasion, not by abstract spiritual state).
    prompt: 'A random selector for THIS run is {{RANDOM:7}}. Use the theme whose number equals it:\n'
      + '0 = seeking forgiveness, 1 = protection from worry and grief, 2 = anger, 3 = travel, '
      + '4 = contentment, 5 = entering the home, 6 = morning and evening.\n'
      + 'Call get_dua with that exact theme as the query. Open with one short line naming the theme and when to '
      + 'say it, then place the dua placeholder. Do not write the dua yourself.',
  },
  {
    id: 'next_prayer',
