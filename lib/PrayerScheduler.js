...
const PRAYER_KEYS_FLOW   = ['Fajr', 'Sunrise', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'];
...
class PrayerScheduler {
  ...
  async _reconcile(destructive) {
    ...
    const enabled = (this.homey.settings.get('advanced') || {}).appEnabled !== false;
    ...
    for (let off = 0; off <= LOOKAHEAD_DAYS; off++) {
      ...
      // 1) AUDIO — one occurrence per prayer per day (no Sunrise for audio).
      ...
      for (const args of instancesBA) {
        const concrete = args.prayerName === 'Any' ? PRAYER_KEYS_FLOW : [args.prayerName];
  ...
  async _fireAudio(name, key, fireAt, dayEpoch) {
    ...
    // Play audio (Sunrise has no adhan — triggers only).
    if (name !== 'Sunrise') {
      try { await this.homey.app.audioRouter.dispatch(name); }
      catch (e) { this.homey.app.logger.error('PrayerScheduler.fireAudio', e, { prayer: name }); }
    }

    // Fire legacy prayer_trigger_all (no condition — fires for every prayer).
    try { await this._cardAll.trigger({ prayerName: name, prayerTime }); }
    catch (e) { this.homey.app.logger.error('PrayerScheduler.triggerAll', e); }

    // Fire legacy prayer_trigger_specific (state-filtered by prayerName).
    try { await this._cardSpecific.trigger({ prayerName: name, prayerTime }, { prayerName: name }); }
  ...
  _buildParams() {
    ...
    const methodKey = CALC_METHODS[calc.method] ? calc.method : 'Dubai';
  ...
  _coords() {
    const locSettings = this.homey.settings.get('location') || {};
    const useHomeyLoc = locSettings.useHomeyLoc !== false;
    const lat = useHomeyLoc
      ? this.homey.geolocation.getLatitude()
      : parseFloat(locSettings.lat || '24.45');
    const lng = useHomeyLoc
      ? this.homey.geolocation.getLongitude()
      : parseFloat(locSettings.lng || '54.37');
