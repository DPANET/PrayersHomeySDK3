...
const RESCHEDULE_DEBOUNCE_MS = 1500;             // collapse bursts (boot churn / rapid saves)
...
class PrayerScheduler {
  ...
  async init() {
    ...
    // Best-effort first pass. Every change source above funnels into the SAME
    // debounced, idempotent rebuild, so the post-boot burst — spurious 'location'
    // churn plus the *real* flow-card 'update' Homey emits once it finishes
    // loading flow arguments — collapses into one rebuild a debounce later, by
    // which point coords and getArgumentValues() are ready. No boot cooldown is
    // needed: the rebuild is idempotent, so absorbing the churn is harmless, and
    // (unlike a cooldown) it never drops the 'update' that registers flow timers.
  ...
  // Debounced, destructive entry point used by event listeners. Bursts of events
  // (boot churn, rapid setting saves, a flow edit firing several 'update's)
  // collapse into a single rebuild; the rebuild is idempotent so coalescing is
  // always safe.
  _scheduleReschedule(delay = RESCHEDULE_DEBOUNCE_MS) {
    ...
    for (let off = 0; off <= LOOKAHEAD_DAYS; off++) {
      ...
      for (const args of instancesBA) {
        ...
        for (const name of concrete) {
          const t = PRAYERS[name];
          if (!t) continue;
          // Honor the global Sunrise opt-out (Advanced settings) — mirrors §1 so
          // before/after flows (incl. "Any") don't fire for Sunrise when excluded.
          if (name === 'Sunrise' && adv.excludeSunrise) continue;
  ...
  async _fireFlow(name, key, fireAt, dayEpoch, state) {
    ...
    // Honor the global Sunrise opt-out (Advanced settings) — mirrors _fireAudio,
    // defense-in-depth in case a Sunrise flow timer was armed before the flag flip.
    const adv = this.homey.settings.get('advanced') || {};
    if (name === 'Sunrise' && adv.excludeSunrise) {
      this.homey.app.logger.debug('PrayerScheduler: Sunrise flow excluded (excludeSunrise=true)');
      return;
    }
    const prayerTimeCalculated = this._fmt(new Date(fireAt));
    let audio = {};
    try { audio = this.homey.app.audioRouter.buildTokens(name); }
    catch (e) { this.homey.app.logger.error('PrayerScheduler.fireFlow.buildTokens', e, { prayer: name }); }
    try {
      await this._cardBA.trigger({ prayerName: name, prayerTimeCalculated, ...audio }, state);
  ...
  // Dedup hash for a before/after timer: direction + offset only (NOT prayerName).
  // The caller appends the concrete prayer + day, so an "Any" flow and an explicit
  // same-prayer/same-offset flow collapse onto ONE timer — the run listener fans
  // both out at fire time, preventing the explicit flow from firing twice.
  _stateHash(args) {
    return `${args.prayerAfterBefore}|${args.prayerDurationTime || 0}|${args.prayerDurationType}`;
  }
