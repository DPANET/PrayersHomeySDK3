...
const BOOT_COOLDOWN_MS      = 90 * 1000;         // ignore spurious boot events this long
...
class PrayerScheduler {
  constructor(homey) {
    ...
    this._suppressedDuringBoot = false; // a real reschedule request was dropped during cooldown
    this._bootCatchup = null;        // one-shot timer that applies a suppressed change after the window
  ...
  async init() {
    ...
    // Suppress event-driven reschedules briefly after boot — Homey fires
    // spurious 'update' and 'location' events while syncing its internal state.
    this._readyAt = Date.now() + BOOT_COOLDOWN_MS;
    // Catch-up: a *legitimate* change can also land in that window (e.g. Homey's
    // geolocation finishing its first sync, or a settings/flow edit). Those are
    // suppressed above, and the ADDITIVE heartbeat can't repair a schedule built
    // from stale coords/method — so arm one destructive reconcile at the end of
    // the window that runs only if something was actually suppressed.
    if (this._bootCatchup) this.homey.clearTimeout(this._bootCatchup);
    this._bootCatchup = this.homey.setTimeout(() => {
      this._bootCatchup = null;
      if (this._suppressedDuringBoot) {
        this._suppressedDuringBoot = false;
        this.homey.app.logger.log('PrayerScheduler: applying change(s) suppressed during boot cooldown');
        this.reschedule();
      }
    }, BOOT_COOLDOWN_MS);
  }
  ...
  _scheduleReschedule(delay = 500) {
    if (Date.now() < this._readyAt) {
      // Inside the boot cooldown: remember that a real request arrived so the
      // catch-up at the end of the window can apply it (see init()).
      this._suppressedDuringBoot = true;
      return;
    }
