...
class IslamicAssistantCard {
    ...
    const runTool = async (name, input) => {
      ...
      try {
        ...
        if (name.startsWith('search_') && res && Array.isArray(res.results)
          ...
          // 1 result from a menu-search tool → auto-resolve to the content item directly.
          // Saves a Claude round-trip and removes the "first result is always fetched" bias.
          // search_hadith_explained is excluded: its results require Claude to judge match quality.
          if (MENU_SEARCH_TOOLS.has(name) && res.results.length === 1) {
            const item = res.results[0];
            if (item && item.selector) {
              const stype = res.type;
              let contentRes = null;
              try {
                if (stype === 'hadith')     contentRes = await tools.getHadith({ id: item.selector.id, language });
                else if (stype === 'quran') contentRes = await tools.getQuran({ surah: item.selector.surah, ayah: item.selector.ayah, language });
                else if (stype === 'fatwa') contentRes = await tools.getFatwa({ id: item.selector.id, language });
                else if (stype === 'dua')   contentRes = await tools.getDua({ query: item.selector.query, language });
              } catch (_) { /* fall through to returning raw single-item list */ }
              if (contentRes && contentRes.block) {
                const toolName = stype === 'quran' ? 'get_quran' : ('get_' + stype);
                const token = '{{BLOCK' + (deferred.length + 1) + '}}';
                deferred.push({ token, block: contentRes.block.trim(), ref: this._refLabel(toolName, contentRes.meta), name: toolName, meta: contentRes.meta || {} });
                moreAvailable = null;  // single result — no "more" hint needed
                return {
                  status: 'fetched',
                  meta: contentRes.meta || {},
                  placeholder: token,
                  note: '1 result found and auto-resolved. Position ' + token + ' in your reply.',
                };
              }
            }
          }
  ...
  async pickSearchItem(chatId, n, language) {
    ...
    // Keep the pending menu alive so the user can scroll back up to the same
    // numbered list and pick OTHER items from the same search (re-selection).
    // Refresh its TTL on each pick so active browsing keeps it from expiring;
    // it still self-expires after SEARCH_EXPIRY_MS of inactivity.
    pending.ts = Date.now();
    state[searchKey] = pending;
    this._saveState(state);
    let res;
    try {
    ...
    const toolName = type === 'quran' ? 'get_quran' : type === 'hadith' ? 'get_hadith'
      : type === 'fatwa' ? 'get_fatwa' : type === 'dua' ? 'get_dua'
      : type === 'surah' ? 'get_surah_link' : ('get_' + type);
    this.updateLastContent(chatId, toolName, res.meta, res.block);
    return { text: res.block, meta: { type: metaType, ...(res.meta || {}) } };
  }

  // Save a directly-fetched content block as the last-shown item so that
  // recall_last_content works correctly after button-tap direct fetches
  // (mi|hadith, vn, vp, qa, tf, ab, dm, pk) that bypass _ask.
  updateLastContent(sender, toolName, meta, block) {
    if (!sender || !block) return;
    const senderKey = 'last_' + String(sender == null ? '' : sender).replace(/\D/g, '');
    const lastKey = LASTCONTENT_KEY + senderKey;
    const state = this._loadState();
    const ref = this._refLabel(toolName, meta || {});
    this._saveLastContent(state, lastKey, [{ name: toolName, ref, meta: meta || {}, block }]);
    this._saveState(state);
  }

  // Return the primary last-shown item for a sender (or null if expired/absent).
  // Used by the ex|h handler to retrieve the hadith text without a Claude round-trip.
  getLastContent(chatId) {
    if (!chatId) return null;
    const senderKey = 'last_' + String(chatId == null ? '' : chatId).replace(/\D/g, '');
    const lastKey = LASTCONTENT_KEY + senderKey;
    const state = this._loadState();
    const c = this._loadLastContent(state, lastKey);
    if (!c || !c.items || !c.items.length) return null;
    return c.items[0];
  }

  // Start a fresh topic search without going through Claude — used by the mr|type|query
  // button to show a pick menu deterministically. Saves pending state so pk/sp buttons work.
  async startSearchFromQuery(chatId, type, query, language) {
    const tools = this.deps.contentTools || loadContentTools();
    const lang = language || 'both';
    let res;
    try {
      if (type === 'hadith')      res = await tools.searchHadith({ query, offset: 0, language: lang });
      else if (type === 'quran')  res = await tools.searchQuran({ query, offset: 0, language: lang });
      else if (type === 'fatwa')  res = await tools.searchFatwa({ query, offset: 0, language: lang });
      else return null;
    } catch (e) { this.logger.error('startSearchFromQuery', e); return null; }

    if (!res || !res.results || !res.results.length) {
      const msg = lang === 'arabic' ? 'لا توجد نتائج.' : 'No results found.';
      return { text: msg, meta: null };
    }

    // 1 result → auto-resolve to content (same logic as the single-result path in _ask).
    if (res.results.length === 1) {
      const item = res.results[0];
      if (item && item.selector) {
        let contentRes = null;
        try {
          if (type === 'hadith')      contentRes = await tools.getHadith({ id: item.selector.id, language: lang });
          else if (type === 'quran')  contentRes = await tools.getQuran({ surah: item.selector.surah, ayah: item.selector.ayah, language: lang });
          else if (type === 'fatwa')  contentRes = await tools.getFatwa({ id: item.selector.id, language: lang });
        } catch (_) { /* fall through */ }
        if (contentRes && contentRes.block) {
          const toolName = type === 'quran' ? 'get_quran' : ('get_' + type);
          const metaType = type === 'quran' ? 'verse' : type;
          this.updateLastContent(chatId, toolName, contentRes.meta, contentRes.block);
          return { text: contentRes.block, meta: { type: metaType, ...(contentRes.meta || {}) } };
        }
      }
    }

    // 2+ results → save pending state and return the numbered pick menu.
    const state = this._loadState();
    const searchKey = this._searchKeyFor(chatId);
    this._savePending(state, searchKey, res);
    this._saveState(state);
