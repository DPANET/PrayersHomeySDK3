...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Fetches a Quran verse from quran.com (Arabic + English), already formatted for sending. The verse '
      ...
      + 'OR pass a "query" topic to find a relevant verse by search. The returned meta carries the surah/ayah so '
      + 'you can call get_tafsir for the same verse.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'The fiqh question to look up; omit for a random fatwa.' },
...
class IslamicAssistantCard {
  ...
  async _ask(prompt, cfg) {
    ...
    const runOne = async (name, input) => {
      ...
      if (name === 'get_quran') return tools.getQuran({ surah: input.surah, ayah: input.ayah, query: input.query, language });
  ...
  _systemPrompt(cfg) {
    ...
    return [
      ...
      '',
      'CONTENT TOOLS (get_hadith, get_hadith_explained, get_quran, get_tafsir, get_dua, get_fatwa):',
      ...
      '  • get_quran(surah,ayah | query): quran.com verse. The meta carries surah/ayah for get_tafsir.',
      '  • get_tafsir(surah,ayah): quran.com commentary (Tazkirul Quran / al-Jalalayn). Use after get_quran.',
      '  • get_dua(query): Hisn al-Muslim adhkar (e.g. "morning and evening", "before sleep", "anger",',
      '    "rain", "travel", "entering the home", "breaking the fast", "istikharah", "forgiveness").',
      '  • get_fatwa(query?, id?): IslamQA.info ruling. Use it for ANY fiqh / "is it permissible" / ruling',
      '    question (pass the question as "query"); no input = random. For a fatwa, output ONLY its placeholder',
      '    with no intro or commentary — the published answer IS the whole reply. You may add at most one short',
      '    line advising to consult a local scholar for a personal situation. If the result reports no answer',
      '    found, say so plainly and advise consulting a scholar — never answer the ruling from memory.',
