...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Fetches ONE authenticated hadith from Sunnah.com — use when the user wants a single hadith '
      ...
      + 'After the content, append ONE short discovery line in the user\'s language, e.g. (EN) "Want more narrations on this? Say \'show me more\'" / (AR) "تريد أحاديث أخرى عن هذا؟ قل \'أرني المزيد\'". '
  ...
  },
  {
    ...
    description: 'Fetches ONE Quran verse from quran.com — use when the user wants a specific verse or one best-matching verse. '
      ...
      + 'After the content, append ONE short discovery line in the user\'s language, e.g. (EN) "Want other verses on this? Say \'show me more\'" / (AR) "تريد آيات أخرى عن هذا؟ قل \'أرني المزيد\'". '
  ...
  },
  {
    ...
    description: 'Fetches ONE dua/adhkar chapter from Hisn al-Muslim. '
      ...
      + 'After the content, append ONE short discovery line in the user\'s language, e.g. (EN) "Want other duas on this topic? Say \'show me more\'" / (AR) "تريد أدعية أخرى؟ قل \'أرني المزيد\'". '
  ...
  },
  {
    ...
    description: 'Fetches an authenticated hadith from HadeethEnc.com WITH its grade (sahih/hasan), a scholarly '
      ...
      + 'After the content, append ONE short discovery line in the user\'s language, e.g. (EN) "Want more explained hadiths on this? Say \'show me more\'" / (AR) "تريد أحاديث أخرى مشروحة؟ قل \'أرني المزيد\'". '
...
class IslamicAssistantCard {
  ...
  _savePending(state, searchKey, res) {
    ...
    const results = isContinuation
      ...
      : res.results;
    // Preserve the raw pool (stable row list for cache-based paging). Once set on
    // page 0 it never shrinks — subsequent pages reuse the same pool.
    const pool = res.pool || (existing && existing.pool) || undefined;
  ...
  async _ask(prompt, cfg, senderKey, state) {
    ...
    try {
      ...
      return { reply: final, meta: this._extractMeta(deferred) };
    } catch (e) {
  ...
  _systemPrompt(cfg) {
    ...
    return [
      ...
      '    the selector map — ALL results seen across pages. "2", "the second", or snippet-matching',
      '    text → resolve to that selector and call get_*. If the number is NOT in the list, ask the',
      '    user to clarify which result they meant. If unrelated to the menu, ignore it entirely.',
      ...
      '    automatically if you forget.) When the user says "more"/"next" AND your previous reply was a',
      '    NUMBERED search menu, call the same search_* with the SAME query and offset increased by 5.',
      '  • BUT if your previous reply was a SINGLE direct item (from get_hadith / get_quran / get_dua /',
      '    get_fatwa — not a numbered menu) and the user asks for "more", start a FRESH search_* for that',
      '    same topic at offset 0 (page 1, items 1–5). Do NOT jump to offset 5 — there was no menu to page.',
