...
class IslamicAssistantCard {
  ...
  // Replace random placeholders with fresh per-run entropy. Both forms resolve
  // to ONE value per run (every {{RANDOM4}} in a prompt gets the SAME number, so
  // the selector line and the option mapping never disagree):
  //   {{RANDOM4}} → integer 0–3        {{RANDOM}} → integer 0–999999999
  ...
  _injectRandom(prompt) {
    ...
    const r4   = String(Math.floor(rng() * 4));
    const rBig = String(Math.floor(rng() * 1e9));
    return prompt
      .replace(/\{\{RANDOM4\}\}/g, r4)
      .replace(/\{\{RANDOM\}\}/g, rBig);
  }
