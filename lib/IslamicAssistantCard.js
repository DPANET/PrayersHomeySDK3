...
const TOOLS = [
  ...
  // ── Search tools (two-step: browse → pick → detail) ───────────────────────
  {
    ...
    description: 'REQUIRED when the user wants to browse/see multiple hadiths on a topic: '
      ...
      + 'When the user names one or more collections ("hadiths about X in Bukhari", "from Bukhari and Muslim", '
      + '"in the six books"), pass them in "books" to scope the search to those collections.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        books:  { type: 'array', items: { type: 'string' },
          description: 'Optional collection scope. Names in any language: Bukhari, Muslim, Abu Dawud, Tirmidhi, '
            + 'Nasai, Ibn Majah, Malik/Muwatta, Ahmad, Darimi — or a group: "sahihayn", "the six books". '
...
// Per-preset tool whitelist for schedule mode. A canonical preset's prompt names
// exactly which tools it uses, so we can send only those schemas instead of all
// 16 — cutting ~3k tokens off every (mostly cold) scheduled call. Used ONLY when
// the prompt still matches the shipped default (_isCanonical); custom or edited
// prompts fall back to the full toolset since they may call anything. Each list
// must cover EVERY tool the preset could legitimately call or the preset breaks.
const SCHEDULE_PRESET_TOOLS = {
  morning_briefing:       ['get_prayer_data', 'get_hadith', 'get_quran', 'get_dua'],
  daily_surprise:         ['get_hadith_explained', 'get_quran', 'get_tafsir', 'get_dua', 'get_fatwa'],
  verse_reflection:       ['get_quran', 'get_tafsir'],
  sunnah_of_the_day:      ['get_hadith'],
  dua_of_the_day:         ['get_dua'],
  next_prayer:            ['get_prayer_data'],
  fatwa_random:           ['get_fatwa'],
  morning_evening_adhkar: ['get_dua'],
  after_prayer_adhkar:    ['get_dua'],
  before_sleep_adhkar:    ['get_dua'],
  adhkar_random:          ['get_dua'],
};
...
class IslamicAssistantCard {
  ...
  async _runSchedule(args, cfg) {
    ...
    // A canonical preset uses a known, fixed set of tools — send only those schemas
    // (smaller cold-call prefix). Custom/edited prompts get the full toolset.
    const toolNames = (!custom && presetId && this._isCanonical(presetId, prompt))
      ? SCHEDULE_PRESET_TOOLS[presetId] : null;

    const reply = await this._ask(prompt, cfg, null, null, toolNames);
  ...
  async _ask(prompt, cfg, senderKey, state, toolNames) {
    ...
    const runOne = async (name, input) => {
      ...
      if (name === 'search_hadith') {
        const res = await tools.searchHadith({ query: input.query, offset: input.offset || 0, books: input.books, language });
    ...
    const runTool = async (name, input) => {
      ...
      try {
        ...
        if (name.startsWith('search_') && res && Array.isArray(res.results)
          ...
          moreAvailable = res.total > shown ? { remaining: res.total - shown } : null;
        }
    ...
    const complete = this.deps.claudeComplete
      ...
      : (opts) => new (loadClaudeClient())({
        ...
        // Keep a conversation's prefix warm for an hour (reply mode); scheduled
        // one-shot calls (no senderKey) use the cheaper 5-min ephemeral cache.
        cacheTtl: senderKey ? '1h' : '5m',
        logger: this.logger,
      }).complete(opts);

    // Restrict the tool schemas to the requested subset (canonical scheduled
    // presets) to shrink the prefix; default to the full set for reply/custom.
    const activeTools = (Array.isArray(toolNames) && toolNames.length)
      ? TOOLS.filter(t => toolNames.includes(t.name))
      : TOOLS;

    const t0 = Date.now();
    try {
      const reply = await complete({ messages, system, tools: activeTools, runTool });
  ...
  _systemPrompt(cfg) {
    ...
    return [
      ...
      '    names a collection ("a hadith about patience in Bukhari"), pass books:["Bukhari"] to scope it.',
      ...
      '  • When the search has MORE results than you showed (its total exceeds the shown page), tell the',
      '    user more are available and that they can reply "more" to see them. (A hint is appended',
      '    automatically if you forget.) When the user says "more"/"next", call the same search_* with',
