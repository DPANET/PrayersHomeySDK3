...
class IslamicAssistantCard {
  ...
  async _ask(prompt, cfg, senderKey, state) {
    ...
    // Records that this turn rendered a numbered search pick-list, so the caller can
    // attach a "More" (next-page) button to the menu — the same way content cards get
    // one. Set only when 2+ results were shown (a real menu, not a skipped single hit).
    let searchMenu = null;
    ...
    const runTool = async (name, input) => {
      ...
      try {
        ...
        if (name.startsWith('search_') && res && Array.isArray(res.results)
          ...
          if (res.results.length >= 2) searchMenu = { type: res.type };
        }
    ...
    try {
      ...
      // search menu, emit a `search` meta so the caller can attach a "More" page button.
      const meta = this._extractMeta(deferred)
        || (searchMenu
          ? { type: 'search', searchType: searchMenu.type,
              hasMore: !!(moreAvailable && moreAvailable.remaining > 0) }
          : null);
  ...
  _systemPrompt(cfg) {
    ...
    return [
      ...
      '  • 2–5 results: render a numbered list. CRITICAL ORDERING RULES:',
      '    (a) Render ALL results the tool returned — do NOT drop, skip, or hide any item.',
      '    (b) Render them in the EXACT SAME ORDER as the tool result. Never reorder by collection',
      '        prestige, relevance, or any other criterion — the tool\'s sequence is canonical.',
      '    (c) Use the EXACT n value from each result object as the label — n is NOT your sequential',
      '        counter; it is a fixed ID used for selection. Renumbering breaks "pick 3" resolution.',
      '    For each result use this layout:',
