...
class AudioRouter {
   ...
   * `volume` is null when the user hasn't set a per-prayer level (device default).
  ...
  buildTokens(prayerName) {
    ...
    if (!this._enabled()) {
      return { adhan_full: '', adhan_short: '', adhkar_morning: '', adhkar_evening: '', quran: '', reciter, custom: '', custom2: '', custom3: '', volume: null };
    }

    const clean = (v, fallback = '') => (typeof v === 'string' && v.trim()) ? v.trim() : fallback;
    const vol = (a.volumes || {})[prayerName];
    const volume = (typeof vol === 'number' && vol >= 0 && vol <= 1) ? vol : null;
