...
class AudioRouter {
   ...
   * Returns one string per URL category and, when set, a number (0-1) for volume.
   * `volume` is OMITTED (key absent) when the user hasn't configured a per-prayer
   * level — Homey's Flow runtime rejects null for number-typed tokens (typeof null
   * === "object"), so we must never emit null here.
  ...
  buildTokens(prayerName) {
    ...
    if (!this._enabled()) {
      return { adhan_full: '', adhan_short: '', adhkar_morning: '', adhkar_evening: '', quran: '', reciter, custom: '', custom2: '', custom3: '' };
    }

    const clean = (v, fallback = '') => (typeof v === 'string' && v.trim()) ? v.trim() : fallback;
    const vol = (a.volumes || {})[prayerName];
    const volumeToken = (typeof vol === 'number' && vol >= 0 && vol <= 1) ? { volume: vol } : {};
