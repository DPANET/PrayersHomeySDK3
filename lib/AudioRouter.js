...
class AudioRouter {
  ...
  buildTokens(prayerName) {
    ...
    if (!this._enabled()) {
      return { adhan_full: '', adhan_short: '', adhkar_morning: '', adhkar_evening: '', quran: '', reciter, custom: '', custom2: '', custom3: '', volume: '' };
    }

    const clean = (v, fallback = '') => (typeof v === 'string' && v.trim()) ? v.trim() : fallback;
    const vol = (a.volumes || {})[prayerName];
    const volume = (typeof vol === 'number' && vol >= 0 && vol <= 1) ? String(vol) : '';
