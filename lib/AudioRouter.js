...
// Fallback volume (0-1) when a prayer has no per-prayer level set ("device
// default"). Homey requires the number token to be present, so we must emit a
// number — 0.3 (30%) is a conservative default that won't blast at full volume.
const DEFAULT_VOLUME = 0.3;

// Fallback recordings used when the user hasn't set a URL.
const DEFAULTS = {
  full:           'https://cdn.aladhan.com/audio/adhans/a1.mp3',
...
class AudioRouter {
   ...
   * back to DEFAULT_VOLUME (0.3). Flows that don't wire the volume tag are unaffected.
  ...
  buildTokens(prayerName) {
    ...
    if (!this._enabled()) {
      return { adhan_full: '', adhan_short: '', adhkar_morning: '', adhkar_evening: '', quran: '', reciter, custom: '', custom2: '', custom3: '', volume: DEFAULT_VOLUME };
    }

    const clean = (v, fallback = '') => (typeof v === 'string' && v.trim()) ? v.trim() : fallback;
    const vol = (a.volumes || {})[prayerName];
    const volume = (typeof vol === 'number' && vol >= 0 && vol <= 1) ? vol : DEFAULT_VOLUME;
