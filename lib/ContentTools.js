 ...
 */
async function searchHadith({ query, offset = 0, language = 'both', fetchImpl } = {}) {
  const f = fetchImpl || defaultFetch();
  const q = (query == null ? '' : String(query)).trim();
  if (!q) return { type: 'hadith', query: q, offset, total: 0, results: [], error: 'query required' };
  try {
    const sid = await mcpInit(f, HADITH_MCP);
    const sr = await mcpCall(f, HADITH_MCP, sid, 'search_hadith', { query: q, mode: 'semantic', limit: 30 });
    const all = ((sr && sr.results) || [])
      .filter(r => r && HADITH_ALLOWED_COLLECTIONS.has(r.collection_slug) && r.hadith_id)
...
module.exports = {
  ...
  searchHadith, searchQuran, searchFatwa, searchDua, searchSurah,
  DUA_CATEGORIES,
