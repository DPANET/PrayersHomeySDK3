...
function formatQuran({ arabic, english, surahName, surahNum, ayahNum, language }) {
  ...
  out.push(`_[quran.com](https://quran.com/${surahNum}/${ayahNum})_`);
  return out.join('\n');
}
 ...
 * @returns {Promise<{ block:string, meta:object }>}
 */
async function getDua({ query, category, language = 'both', rng } = {}) {
  const r = rng || Math.random;
  const input = query || category;
  // No input → open random adhkar across the entire Hisn al-Muslim corpus.
  const slug = input
...
function formatTafsir({ text, surahNum, ayahNum, edition }) {
  ...
  return ['📖 *' + name + ` — ${surahNum}:${ayahNum}*`, '', text, '', `_[quran.com](https://quran.com/${surahNum}/${ayahNum})_`].join('\n');
}
 ...
 */
async function searchHadith({ query, offset = 0, books, language = 'both', fetchImpl } = {}) {
  const f = fetchImpl || defaultFetch();
  const q = (query == null ? '' : String(query)).trim();
  ...
  try {
    const sid = await mcpInit(f, HADITH_MCP);
    // A book filter narrows the pool, so widen the fetch when one is active to keep
    // enough results to fill (and page) the menu.
    const limit = bookSet ? 60 : 30;
    const sr = await mcpCall(f, HADITH_MCP, sid, 'search_hadith', { query: q, mode: 'semantic', limit });
    const all = ((sr && sr.results) || [])
      .filter(r => r && allow.has(r.collection_slug) && r.hadith_id)
      .sort((a, b) => (b.similarity || 0) - (a.similarity || 0));

    const page = all.slice(offset, offset + SEARCH_RESULTS_MAX);
    if (!page.length) return { type: 'hadith', query: q, offset, total: all.length, results: [] };
    ...
    return { type: 'hadith', query: q, offset, total: all.length, results };
  } catch (e) {
 ...
 */
async function searchQuran({ query, offset = 0, language = 'both', fetchImpl } = {}) {
  const f = fetchImpl || defaultFetch();
  const q = (query == null ? '' : String(query)).trim();
  if (!q) return { type: 'quran', query: q, offset, total: 0, results: [], error: 'query required' };
  try {
    const sid = await mcpInit(f, QURAN_MCP);
    await mcpGround(f, QURAN_MCP, sid);
    const sr = await mcpCall(f, QURAN_MCP, sid, 'search_quran', { query: q, translations: [QURAN_TRANSLATION_EDITION] });
    ...
    const page = all.slice(offset, offset + SEARCH_RESULTS_MAX);
    if (!page.length) return { type: 'quran', query: q, offset, total: all.length, results: [] };
    ...
    return { type: 'quran', query: q, offset, total: all.length, results };
  } catch (e) {
 ...
 */
async function searchFatwa({ query, offset = 0, language = 'both', fetchImpl } = {}) {
  const f = fetchImpl || defaultFetch();
  const q = (query == null ? '' : String(query)).trim();
  if (!q) return { type: 'fatwa', query: q, offset, total: 0, results: [], error: 'query required' };
  try {
    const sid = await mcpInit(f, ISLAMQA_MCP);
    const sr = await mcpCall(f, ISLAMQA_MCP, sid, 'search_answers', { query: q, mode: 'semantic', limit: 30 });
    const all = ((sr && sr.results) || []).filter(r => r && r.answer_id != null)
      .sort((a, b) => (b.similarity || 0) - (a.similarity || 0));

    const page = all.slice(offset, offset + SEARCH_RESULTS_MAX);
    if (!page.length) return { type: 'fatwa', query: q, offset, total: all.length, results: [] };
    ...
    return { type: 'fatwa', query: q, offset, total: all.length, results };
  } catch (e) {
 ...
 */
async function searchDua({ query, offset = 0, language = 'both' } = {}) {
  const q = (query == null ? '' : String(query)).trim();
  if (!q) return { type: 'dua', query: q, offset, total: 0, results: [], error: 'query required' };
  ...
  if (!page.length) return { type: 'dua', query: q, offset, total, results: [] };
  ...
  return { type: 'dua', query: q, offset, total, results };
}
