...
async function getQuran({ surah, ayah, query, language = 'both', tafsir, fetchImpl, rng } = {}) {
  ...
  try {
    ...
    const meta = { surahName, surahNum: s, ayahNum: a, language, source: 'quran.com', query: (viaSearch && q) ? q : null };
...
async function getDua({ query, category, language = 'both', rng, offset = 0 } = {}) {
  ...
  return {
    ...
    meta:  { category: slug, title: cat.title, offset, count: shown, total: entries.length, nextOffset, language, query: wantRandom ? null : (input || null) },
  };
}
...
async function searchHadith({ query, offset = 0, books, language = 'both', fetchImpl, pool } = {}) {
  ...
  try {
    ...
    if (pool && Array.isArray(pool) && pool.length) {
      ...
      // Unscoped search uses the MCP's max (100): the corpus has 17 collections but
      // only 8 are allowed here, and for some topics non-allowed collections (Darimi,
      // Mishkat, Riyad…) dominate the top of the semantic ranking. A small fetch left
      // the allowed Six Books / Malik / Ahmad crowded out — e.g. "صدقة" at limit 30
      // surfaced 7 results with no Ibn Majah; at 100 it surfaces ~44 across 6 books.
      const limit = bookSet ? (bookSet.size > 1 ? 80 : 60) : 100;
      const sr = await mcpCall(f, HADITH_MCP, sid, 'search_hadith', { query: q, mode: 'semantic', limit });
      const rows = ((sr && sr.results) || [])
