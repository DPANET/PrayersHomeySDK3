...
class ClaudeClient {
   ...
   */
  constructor({ apiKey, model, fetchImpl, timeoutMs, maxTokens, maxToolRounds, cacheTtl, logger } = {}) {
    this.apiKey        = apiKey;
    ...
    // Prompt-cache lifetime for the static prefix (tools+system). '1h' keeps the
    // prefix warm across a back-and-forth conversation (reply mode); the default
    // 5-min ephemeral suits one-shot scheduled calls (write + read within the
    // single tool loop, no cross-call reuse to amortise a pricier 1h write).
    this.cacheTtl      = cacheTtl === '1h' ? '1h' : '5m';
    ...
    try {
      const res = await this.fetchImpl(API_URL, {
        ...
        headers: {
          ...
          // Extended (1h) cache TTL is gated behind a beta header; only send it
          // when actually requesting a 1h prefix cache.
          ...(this.cacheTtl === '1h' ? { 'anthropic-beta': 'extended-cache-ttl-2025-04-11' } : {}),
        },
  ...
  async complete({ messages, system, tools, runTool }) {
    ...
    if (system) {
      const cc = this.cacheTtl === '1h' ? { type: 'ephemeral', ttl: '1h' } : { type: 'ephemeral' };
      base.system = [{ type: 'text', text: system, cache_control: cc }];
    }
